...
func (c *fragmentConn) Write(b []byte) (n int, err error) {
  ...
  for fragmentStart := 0; fragmentStart < clientHelloLen; {
    fragmentEnd := fragmentStart + c.fragment.SizeMin + rand.Intn(c.fragment.SizeMax-c.fragment.SizeMin+1)
    ...
    if c.fragment.SleepMax != 0 {
      time.Sleep(time.Duration(c.fragment.SizeMin + rand.Intn(c.fragment.SizeMax-c.fragment.SizeMin+1)) * time.Millisecond)
    }
